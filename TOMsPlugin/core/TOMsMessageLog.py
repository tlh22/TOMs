...
from qgis.core import Qgis, QgsExpressionContextUtils, QgsMessageLog, QgsProject
...
class TOMsMessageLog(QgsMessageLog):
    ...
    def logMessage(*args, **kwargs):
        ...
        def currentLoggingLevel():
            ...
            try:
                currLoggingLevel = int(
                    QgsExpressionContextUtils.projectScope(
                        QgsProject.instance()
                    ).variable("TOMs_Logging_Level")
                )
            except Exception as e:
                QgsMessageLog.logMessage(
                    "Error in TOMsMessageLog. TOMs_logging_Level not found ... {}".format(
                        e
                    ),
                    tag="TOMs Panel",
                )
        ...
        if TOMsMessageLog.filename:
            try:
                ...
                QgsMessageLog.logMessage(
                    "Error in TOMsMessageLog. Level in message not found...{}".format(
                        e
                    ),
                    tag="TOMs Panel",
                )
    ...
    def setLogFile(self):
        ...
        if logFilePath:
            ...
            QgsMessageLog.logMessage(
                "Sorting out log file" + self.filename,
                tag="TOMs Panel",
                level=Qgis.Info,
            )
