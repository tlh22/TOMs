...
class TOMsLabelTool(TOMsNodeTool):
        ...
        if currRestrictionLayer.name() == "Lines":
            ...
            TOMsMessageLog.logMessage(
                "In doEditLabels - result: {}. {}".format(
                    msgBox.clickedButton().text(), btn_clicked
                ),
                level=Qgis.Warning,
            )
    ...
    def snap_to_editable_layer(self, e):
        ...
        for label_layers_name in self.getCurrLabelLayerNames(self.origLayer):
            snap_layers.append(
                QgsProject.instance().mapLayersByName(label_layers_name)[0]
            )
            # add signal here
        ...
        filter_last = OneFeatureLabelFilter(
            self.origLayer, self.origFeature.getFeature().id(), self.label_layers_names
        )
        # m = snap_util.snapToMap(map_point, filter_last)
        """if m_last.isValid() and m_last.distance() <= m.distance():
            m = m_last"""
