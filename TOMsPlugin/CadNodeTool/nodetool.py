...
from .geomutils import (
    adjacent_vertex_index_to_endpoint,
    is_endpoint_at_vertex_index,
    vertex_at_vertex_index,
    vertex_index_to_tuple,
)
...
def _is_circular_vertex(geom, vertex_index):
    ...
    if (
        ...
        return False
    v_id = QgsVertexId()
    res, v_id = geom.vertexIdFromVertexNr(
        vertex_index
...
class NodeTool(QgsMapToolAdvancedDigitizing):
    def __init__(self, canvas, cadDock):
        ...
        self.dragging_rect_start_pos = (
            None  # QPoint if user is dragging a selection rect
        )
    ...
    def cadCanvasMoveEvent(self, e):
        ...
        if self.dragging:
            ...
            if (
                self.selection_rect is None
                and (e.pos() - self.dragging_rect_start_pos).manhattanLength() >= 10
            ):
    ...
    def mouse_move_dragging_edge(self, e):
        ...
        diff_x, diff_y = (
            map_point.x() - drag_start_point.x(),
            map_point.y() - drag_start_point.y(),
        )
        ...
        new_map_point_0 = QgsPointXY(
            orig_map_point_0.x() + diff_x, orig_map_point_0.y() + diff_y
        )
        orig_map_point_1 = self.toMapCoordinates(
            drag_layer, geom.vertexAt(drag_vertex_0 + 1)
        )
        orig_map_point_1 = QgsPointXY(geom.vertexAt(drag_vertex_0 + 1))
        new_map_point_1 = QgsPointXY(
            orig_map_point_1.x() + diff_x, orig_map_point_1.y() + diff_y
        )
    ...
    def snap_to_editable_layer(self, e):
        ...
        if self.last_snap is not None:
            filter_last = OneFeatureFilter(
                self.last_snap.layer(), self.last_snap.featureId()
            )
    ...
    def is_near_endpoint_marker(self, map_point):
        ...
        dist_marker = math.sqrt(
            self.endpoint_marker_center.sqrDist(map_point.x(), map_point.y())
        )
        tol = QgsTolerance.vertexSearchRadius(self.canvas().mapSettings())
    ...
    def position_for_endpoint_marker(self, match):
        ...
        pt0 = vertex_at_vertex_index(
            geom, adjacent_vertex_index_to_endpoint(geom, match.vertexIndex())
        )
    ...
    def mouse_move_not_dragging(self, e):
        ...
        if m.type() == QgsPointLocator.Vertex:
            ...
            self.vertex_band.setIcon(
                QgsRubberBand.ICON_FULL_BOX
                if is_circular_vertex
            ...
            if self.is_match_at_endpoint(m):
                self.mouse_at_endpoint = Vertex(
                    m.layer(), m.featureId(), m.vertexIndex()
                )
    ...
    def start_dragging_move_vertex(self, map_point, m):
        ...
        for layer in self.canvas().layers():
            ...
            for other_m in self.layer_vertices_snapped_to_point(layer, map_point):
                ...
                # start dragging of snapped point of current layer
                self.dragging_topo.append(
                    Vertex(other_m.layer(), other_m.featureId(), other_m.vertexIndex())
                )
    ...
    def layer_vertices_snapped_to_point(self, layer, map_point):
        ...
        class MyFilter(QgsPointLocator.MatchFilter):
            ...
            def acceptMatch(self, match):
                ...
                match_geom = self.nodetool.cached_geometry(
                ...
                while geomIter.hasNext():
                    ...
                    if pt.x() == match.point().x() and pt.y() == match.point().y():
                        extra_match = QgsPointLocator.Match(
                            match.type(),
                            match.layer(),
                            match.featureId(),
                            0,
    ...
    def start_dragging_add_vertex_at_endpoint(self, map_point):
        ...
        self.dragging = Vertex(
            self.mouse_at_endpoint.layer,
            self.mouse_at_endpoint.fid,
            (self.mouse_at_endpoint.vertex_id, True),
        )
        self.dragging_topo = []

        geom = self.cached_geometry(
            self.mouse_at_endpoint.layer, self.mouse_at_endpoint.fid
        )
        ...
        pt0 = vertex_at_vertex_index(
            geom,
            adjacent_vertex_index_to_endpoint(geom, self.mouse_at_endpoint.vertex_id),
        )
    ...
    def match_to_layer_point(self, dest_layer, map_point, match):
        ...
        if (
            ...
            and match.hasVertex()
            and match.layer()
            and match.layer().crs() == dest_layer.crs()
        ):
            try:
    ...
    def move_vertex(self, map_point, map_point_match):
        ...
        if adding_vertex:
            ...
            drag_part, drag_ring, drag_vertex = vertex_index_to_tuple(
                geom, drag_vertex_id
            )
            if adding_at_endpoint and drag_vertex != 0:  # appending?
                drag_vertex += 1
            vid = QgsVertexId(
                drag_part, drag_ring, drag_vertex, QgsVertexId.SegmentVertex
            )
    ...
    def delete_vertex(self):
        ...
        for layer, features_dict in to_delete_grouped.items():
            ...
            for fid, vertex_ids in features_dict.items():
                ...
                for vertex_id in sorted(vertex_ids, reverse=True):
                    ...
                    if (
                        res != QgsVectorLayer.EmptyGeometry
                        and res != QgsVectorLayer.Success
                    ):
                        print(
        ...
        if len(to_delete) == 1:
            ...
            if geom.vertexAt(vertex.vertex_id) != QgsPoint():
                self.set_highlighted_nodes(
                    [Vertex(vertex.layer, vertex.fid, vertex.vertex_id)]
                )
    ...
    def _match_edge_center_test(self, m, map_point):
        ...
        dist_from_edge_center = math.sqrt(
            map_point.sqrDist(edge_center.x(), edge_center.y())
        )
        tol = QgsTolerance.vertexSearchRadius(self.canvas().mapSettings())
