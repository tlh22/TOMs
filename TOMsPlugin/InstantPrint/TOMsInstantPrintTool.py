...
from .ui.printList_dialog import printListDialog as printListDialogUI
...
class TOMsInstantPrintTool(InstantPrintTool):
    def __init__(self, iface, proposalsManager):
        ...
        try:
            self.dialogui.comboBox_scale.scaleChanged.disconnect(
    ...
    def TOMsChangeScale(self):
        ...
        layout = self.dialogui.comboBox_layouts.itemData(
            self.dialogui.comboBox_layouts.currentIndex()
        )
        if layout.atlas().enabled():
            if self.rubberband:
                self.iface.mapCanvas().scene().removeItem(self.rubberband)
