...
class InstantPrintPlugin():
    def __init__(self, iface, TOMsSearchBar):
        #QObject.__init__(self)

        self.iface = iface
        #self.pluginDir = os.path.dirname(__file__)  # removed TH (180606)
        self.tool = InstantPrintTool(self.iface)
        self.TOMsSearchBar = TOMsSearchBar

        # Localize  # Removed TH (180606)
        """locale = QSettings().value("locale/userLocale")[0:2]
        localePath = os.path.join(self.pluginDir, 'i18n', 'instantprint_{}.qm'.format(locale))
    ...
    def initGui(self):
        ...
        #self.toolButton.setToolTip(self.tr("Instant Print"))
        self.toolButton.setCheckable(True)
        self.toolAction = self.TOMsSearchBar.addWidget(self.toolButton)
        #self.toolAction = self.iface.pluginToolBar().addWidget(self.toolButton)
    ...
    def unload(self):
        ...
        self.tool = None
        self.iface.TOMsSearchBar().removeAction(self.toolAction)
