...
SET "TimePeriodID" =
               CASE
                 WHEN "TimePeriodID" = 12 THEN 12
                 WHEN "TimePeriodID" = 14 THEN 14
                 WHEN "TimePeriodID" = 33 THEN 309
                 WHEN "TimePeriodID" = 39 THEN 308
                 WHEN "TimePeriodID" = 97 THEN 97
                 ...
                 WHEN "TimePeriodID" = 217 THEN 217
                 --ELSE "TimePeriodID"
               END,
...
WHERE r."RestrictionID" IN (
    ...
    AND r."TimePeriodID" NOT IN (0, 1, 306, 307, 308, 309, 312, 314, 316, 317, 318)
...
SET "NoWaitingTimeID" =
               CASE
                 WHEN "NoWaitingTimeID" = 12 THEN 12
                 WHEN "NoWaitingTimeID" = 14 THEN 14
                 WHEN "NoWaitingTimeID" = 33 THEN 309
                 WHEN "NoWaitingTimeID" = 39 THEN 308
                 WHEN "NoWaitingTimeID" = 97 THEN 97
                 ...
                 WHEN "NoWaitingTimeID" = 217 THEN 217
                 --ELSE "NoWaitingTimeID"
               END,
      "Notes" = CONCAT ("Notes", ' Sunday Parking')
WHERE r."RestrictionID" IN (
    SELECT r."RestrictionID"
    ...
    WHERE ST_Intersects(r.geom, c.geom)
    AND c."CPZ" IN ('1', '1A', '2', '3', '4')
    AND r."NoWaitingTimeID" NOT IN (0, 1, 306, 307, 308, 309, 312, 314, 316, 317, 318)
    AND r."OpenDate" IS NOT NULL
    AND r."CloseDate" IS NULL
    AND r."RestrictionID" IN (SELECT "RestrictionID"
                              FROM toms."RestrictionsInProposals" RiP
